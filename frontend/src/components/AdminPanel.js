...
const AdminPanel = () => {
  ...
  const mainAdminEmail = process.env.REACT_APP_MAIN_ADMIN_EMAIL;
  ...
  const fetchAuthorizedUsers = async () => {
    ...
    try {
      const response = await api.get(API_ENDPOINTS.AUTHORIZED_USERS);
  ...
  const toggleAdminStatus = async (userEmail) => {
    ...
    try {
      await api.post(API_ENDPOINTS.TOGGLE_ADMIN, {
  ...
  const revokeAccess = async (userEmail) => {
    try {
      await api.post(API_ENDPOINTS.REVOKE_ACCESS, {
